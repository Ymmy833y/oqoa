...
class QListDB extends BaseDB<QList> {
  constructor() {
    super('q_list');
  }
  ...
  initStore(event: IDBVersionChangeEvent): void {
    ...
    if (!db.objectStoreNames.contains(this.storeName)) {
      ...
      store.createIndex('questions', 'questions', { unique: false, multiEntry: true });
   ...
   */
  async bulkInsert(qLists: QList[]): Promise<{ inserted: number; skipped: number }> {
    ...
    console.log(`QListDB.bulkInsert: inserted=${inserted}, skipped=${skipped}`);
  ...
  async selectByNameAndQuestions(name: string, questions: number[]): Promise<QList | undefined> {
...
export const qListDB = new QListDB();
