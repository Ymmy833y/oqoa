...
export async function generatePracticeDetailDto(
  ...
  const practiceHistoryId = await practiceHistoryRepository.insert(
  ...
  const questionDetailDtos = await Promise.all(
    qList
      .getQuestions()
      .map((q) => selectQuestionDetailDto(q, practiceHistoryId)),
  );
...
export async function generatePracticeDetailDtoForCustom(
  dto: CustomPracticeStartDto,
  isShuffleQuestions: boolean,
  isShuffleChoices: boolean,
  questionCount: number,
): Promise<PracticeDetailDto> {
  const questionIds = (
    isShuffleQuestions ? shuffled(dto.questionIds) : dto.questionIds
  ).slice(0, questionCount);
  const qList = QList.fromCreateNew(dto.name, questionIds, false);
  const qListId = await qListRepository.insert(qList.generateRow());
  qList.setId(qListId);

  return generatePracticeDetailDto(
    qList,
    isShuffleQuestions,
    isShuffleChoices,
    dto.isReview,
...
export async function generatePracticeDetailDtoForExist(
  practiceHistoryId: number,
): Promise<PracticeDetailDto> {
  const practiceHistory =
    await practiceHistoryRepository.selectById(practiceHistoryId);
...
export async function selectPracticeHistoryDtos(
  form: PracticeHistorySearchForm,
): Promise<{
  ...
  const practiceHistories =
    await practiceHistoryRepository.selectAllOrderDesc();

  const totalPages =
    Math.floor(practiceHistories.length / PAGE_ITEM_SIZE) +
    (practiceHistories.length % PAGE_ITEM_SIZE === 0 ? 0 : 1);
  ...
  const currentPracticeHistories = practiceHistories.slice(
    startIndex,
    endIndex,
  );
  ...
  return {
    practiceHistoryDtos,
    currentPage,
    totalSize: practiceHistories.length,
    pages,
  };
}
