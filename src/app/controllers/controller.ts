...
export class Controller {
  ...
  private registerViewHandlers() {
    this.view.on(UIEvent.CLICK_SETTING_BTN, () =>
      this.dispatch({
        type: Action.OPEN_DEFAULT_MODAL,
        kind: ModalKind.SETTING,
      }),
    );
    ...
    this.view.on(UIEvent.CLICK_REMOVE_QUESTION_BTN, () =>
      this.dispatch({ type: Action.REMOVE_LOCAL_STORAGE_QUESTION }),
    );
    this.view.on(
    ...
    this.view.on(
      ...
      ({ practiceHistoryId, questionId, isCorrect, selectChoice }) =>
        this.dispatch({
    ...
    this.view.on(UIEvent.TOGGLE_QLISTS_STANDARD_CHECK, ({ standardOnly }) =>
      this.dispatch({
        type: Action.TOGGLE_QLISTS_STANDARD_CHECK,
        standardOnly,
      }),
    );
    ...
    for (const fx of effects) {
      switch (fx.kind) {
        case Effect.INIT_REPOSITORY: {
          await indexeddbService
            ...
            .catch((error) => {
              ...
              this.dispatch({
                ...
                toastMessage: generateErrorToastMessage(
          ...
          const questionData =
            await questionService.selectQuestionsForSearchForm(
              this.model.questionSearchForm,
            );
          this.dispatch({
            type: Action.UPDATE_QUESTION_LIST_CONTAINER,
            questions: questionData.questions,
            currentPage: qListData.currentPage,
            totalSize: questionData.totalSize,
        ...
        case Effect.IMPORT_GOOGLE_DRIVE: {
          const toastMessage =
            await importGoogleDriveService.importProbelmForGoogleDrive(
              fx.googleClientId,
              fx.googleFolderId,
            );
        ...
        case Effect.REMOVE_LOCAL_STORAGE_QUESTION: {
          setLastUsedQuestions([]);
          this.dispatch({
            type: Action.TOAST_ADD,
            toastMessage: generateSuccessToastMessage(
              "設問データのキャッシュを削除しました。",
            ),
          });
          break;
        ...
        case Effect.SEARCH_QLIST: {
          const { qLists, currentPage, totalSize, pages } =
            await qListService.selectQListsForSearchForm(
              this.model.qListSearchForm,
            );
          this.dispatch({
            type: Action.UPDATE_QLIST_CONTAINER,
        ...
        case Effect.SEARCH_QUESTION: {
          try {
            const { questions, currentPage, totalSize, pages } =
              await questionService.selectQuestionsForSearchForm(
                this.model.questionSearchForm,
              );
            this.dispatch({
            ...
            this.dispatch({
              type: Action.TOAST_ADD,
              toastMessage: generateErrorToastMessage(error),
            });
          }
          break;
        }
        ...
        case Effect.PREPARE_PRACTICE: {
          const practiceDetailDto =
            fx.preparePracticeStart instanceof QList
              ? await practiceSevice.generatePracticeDetailDto(
                  fx.preparePracticeStart,
                  fx.isShuffleQuestions,
                  fx.isShuffleChoices,
                )
        ...
        case Effect.PREPARE_EXIST_PRACTICE: {
          const practiceDetailDto =
            await practiceSevice.generatePracticeDetailDtoForExist(
        ...
        case Effect.PREPARE_QUESTION_DETAIL: {
          try {
            const questionDetailDto =
              await questionService.selectQuestionDetailDto(
                fx.questionId,
                fx.practiceHistoryId,
              );
            this.dispatch({
              type: Action.SHOW_QUESTION_DETAIL,
              questionDetailDto,
            });
          } catch (e) {
            const error = e as Error;
        ...
        case Effect.COMPLETE_ANSWER: {
          ...
          try {
            const practiceDetailDto = await practiceSevice.doPracticeComplete(
        ...
        case Effect.PRACTICE_ANSWER_CANCELED: {
          ...
          const { practiceDetailDto, toastMessage } =
            await ansHistoryService.cancelAnsHistory(
              this.model.practiceDetailDto,
              fx.practiceHistoryId,
              fx.questionId,
            );
        ...
        case Effect.SEARCH_PRACTICE_HISTORY: {
          const { practiceHistoryDtos, currentPage, totalSize, pages } =
            await practiceSevice.selectPracticeHistoryDtos(
        ...
        case Effect.SEARCH_ANS_HISTORY: {
          const { ansHistoryDtos, currentPage, totalSize, pages } =
            await ansHistoryService.selectAnsHistoryDtos(
        ...
        case Effect.PREPARE_CUSTOM_PRACTICE: {
          ...
          if (questions.length <= 0) {
            this.dispatch({
              type: Action.TOAST_ADD,
              toastMessage: generateErrorToastMessage(
                "カスタム問題集の作成には、設問一覧に1問以上の設問を表示させる必要があります。",
              ),
            });
          } else {
            this.dispatch({
        ...
        case Effect.UPDATE_FAVORITE: {
          const practiceDetailDto = await favoriteService.upsertFavorite(
            fx.questionId,
            fx.tagId,
            fx.checked,
            this.model.practiceDetailDto,
