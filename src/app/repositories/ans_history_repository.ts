...
class AnsHistoryRepository extends BaseRepository<AnsHistory> {
  constructor() {
    super("ans_history");
  }
  ...
  async selectByPracticeHistoryId(
  ...
  async selectAllOrderDesc(): Promise<AnsHistory[]> {
    return (await this.selectAll()).sort((p1, p2) => p2.getId() - p1.getId());
  }
  ...
  async hasAnsHistory(
    ...
    const results = await this.selectExample(
      "practice_history_id",
      practiceHistoryId,
    );
    return results.some(
      (ansHistory) => ansHistory.getQuestionId() === questionId,
    );
  }

  async hasCorrectAnsHistory(
    practiceHistoryId: number,
    questionId: number,
  ): Promise<boolean> {
    const results = await this.selectExample(
    ...
    return results.some(
      (ansHistory) =>
        ansHistory.getIsCorrect() && ansHistory.getQuestionId() === questionId,
    );
  }
...
export const ansHistoryRepository = new AnsHistoryRepository();
