...
class PracticeHistoryRepository extends BaseRepository<PracticeHistory> {
  ...
  async selectAllOrderDesc(): Promise<PracticeHistory[]> {
    return (await this.selectAll()).sort(
      (p1, p2) => Date.parse(p2.getCreateAt()) - Date.parse(p1.getCreateAt()),
    );
  }
  ...
  async selectByQListIdAndCreateAt(
    qListId: number,
    createAt: string,
  ): Promise<PracticeHistory | undefined> {
    const practiceHistories = await this.selectByQListId(qListId);
    const filteredPracticeHistories = practiceHistories.filter(
      (ph) => ph.getCreateAt() === createAt,
    );
