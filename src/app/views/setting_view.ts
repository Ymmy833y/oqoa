...
export function generateSettingModalConetnt(
  ...
  radioGroup.appendChild(
    createThemeRadio(Theme.LIGHT, "ライト", Theme.LIGHT === theme),
  );
  radioGroup.appendChild(
    createThemeRadio(Theme.DARK, "ダーク", Theme.DARK === theme),
  );
  ...
  const driveTitle = el("h3", "app-modal-section-title", "Google Drive 連携");
  const driveDesc = el(
    "p",
    "app-modal-section-description",
  ...
  // 設問セクション
  const questionSection = el("section", "app-modal-section");
  const questionTitle = el("h3", "app-modal-section-title", "設問削除");
