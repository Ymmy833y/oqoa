...
export class Modal {
  ...
  public initModal(modalElem: HTMLElement) {
    if (this.modalElem) {
      return;
    }
    this.modalElem = modalElem;
  ...
  /**
   * モーダルの初期化
   * @param content モーダル要素に設定する属性
   * @param size サイズ指定（"lg", "xl", "fl-lg", "fl" など）
  ...
  private generateDialogElem(): HTMLElement {
    ...
    modal.className = 'relative bg-white dark:bg-gray-800 p-6 rounded shadow-lg max-h-[90vh] overflow-y-auto';
    return modal;
  }
  ...
  private generateCloseBtn(): HTMLElement {
    ...
    closeBtn.className = 'w-8 h-8 rounded-[1vw] border border-gray-400 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-700 flex items-center justify-center transition-colors';
    closeBtn.innerText = '✕';
...
export const defaultModal = new Modal();
